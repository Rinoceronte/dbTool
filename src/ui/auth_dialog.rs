...
pub fn draw(ctx: &egui::Context, state: &mut AuthState) -> AuthAction {
    if !state.open {
    ...
    egui::Window::new("Claude account")
        .collapsible(false)
        .resizable(false)
        .default_width(440.0)
        .open(&mut open)
        .show(ctx, |ui| {
            ui.label("dbTool's AI agent runs through your local `claude` CLI, \
