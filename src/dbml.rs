...
/// Render introspected table structures as DBML source. When tables span
/// multiple schemas, names are schema-qualified and each schema becomes a
/// TableGroup; a single-schema database stays unqualified and ungrouped.
pub fn generate(tables: &[TableStructure], source_name: &str) -> String {
    use std::fmt::Write;
    ...
    let _ = writeln!(
        out,
        "// Generated by dbTool from {source_name} — {} table(s)\n",
        tables.len()
    );
    ...
    if qualified {
        for schema in &schemas {
            ...
            for t in tables.iter().filter(|t| t.schema == *schema) {
                let _ = writeln!(out, "  {}", table_ident(t));
            }
            let _ = writeln!(out, "}}\n");
        }
    }
...
mod tests {
    ...
    fn generated_dbml_round_trips_through_parse() {
        ...
        let src = generate(&[users, orders], "test-db");
    ...
    fn generate_single_schema_stays_unqualified() {
        ...
        let src = generate(&[t], "db");
        assert!(src.contains("Table users {"));
        assert!(!src.contains("TableGroup"));
    }
